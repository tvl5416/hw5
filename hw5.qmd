...
NNet <- nn_module(
  initialize = function(p, q1, q2, q3) {
    ...
    architecture <- list(
      layer("input", size = p),  # Input layer with size 'p'
      layer("fully_connected", size = q1),  # Fully connected layer with 'q1' nodes
      layer("fully_connected", size = q2),  # Fully connected layer with 'q2' nodes
      layer("fully_connected", size = q3),  # Fully connected layer with 'q3' nodes
      layer("output", size = 1)  # Output layer with size 1
    )
    return(architecture)
  },
  forward = function(x) {
...
nnet_fit <- NNet %>%
  setup(
    input_size = ncol(train_data),  # Number of input features
    q1 = 64,  # Number of nodes in the first hidden layer
    q2 = 32,  # Number of nodes in the second hidden layer
    q3 = 16   # Number of nodes in the third hidden layer
  ) %>%
  set_hparams(
