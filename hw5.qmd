...
result <- nnet_fit %>%
  fit(
    x = model.matrix(~., data = df_train), 
    y = df_train$median_house_value, 
    validation_split = 0.2, 
    verbose = FALSE # Set to TRUE for tuning
  )
```
...
nnet_predictions <- as.vector(as.array(nnet_predictions))

rmse(nnet_predictions, df_test$median_house_value)
```
...
```{R}
glm_fit <- glm(spam ~ ., data = df1_train, family = "binomial")
...
overview(pred_class = glm_classes, true_class = df1_test$spam)$accuracy
```
...
rpart_classes <- predict(dt_model, newdata = df1_test, type = "class")
...
```{R}
overview(pred_class = rpart_classes, true_class = df1_test$spam)$accuracy
```
...
```{R}
svm_fit <- svm(spam ~ ., data = df1_train, type = "C-classification", kernel = "radial")
```
...
overview(pred_class = svm_classes, true_class = df1_test$spam)$accuracy
```
...
```{R}
X_train <- model.matrix(~ . -1, data = df1_train)  # Create design matrix, excluding intercept
y_train <- ifelse(df1_train$spam == "spam", 1, 0)  # Ensure binary encoding

X_test <- model.matrix(~ . -1, data = df1_test)
y_test <- ifelse(df1_test$spam == "spam", 1, 0)
...
# Assuming 'model' is your neural network model defined as per previous instructions
model_fit <- model %>%
  setup(
    loss = nn_bce_loss(),
  ...
  fit(
    ...
    epochs = 100,
    valid_data = list(X_test_tensor, y_test_tensor),
    verbose = TRUE
  )
```
